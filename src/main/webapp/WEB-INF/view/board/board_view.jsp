...
<script type="text/javascript">
   ...
   $.each(res, function(index, value) {
		...
		var source 	= 	'<div class="time_sub" id="{{reply_num}}">'
			...
			+		'</div>';
				if(value.reply_writer == mem_id){ 		/* 댓글작성자 == 로그인ID */
					source = source + '<div>'
					+				'<button id="{{reply_num}}" class = "deletebutton" style = "margin-right : 6px;">삭제</button>'
					+				'<button id="{{reply_num}}" class = "updatebutton">수정</button>'
					+			'</div>'
					+ 		'</div>';
				}else if(value.reply_writer == kemail){
					//alert(kemail);
					source = source + '<div>'
					+				'<button id="{{reply_num}}" class = "deletebutton" style = "margin-right : 6px;">삭제</button>'
					+				'<button id="{{reply_num}}" class = "updatebutton">수정</button>'
...
<div id = all>
   ...
   <form name="frm" id="frm" method="get">
      ...
      <div id="multibutton">
		...
		<c:if test="${dto.board_writer == sessionScope.mem_id}">
      	...
      	<!-- 카카오 -->
		<!-- 수정 권한 (작성자) -->
		<c:if test="${dto.board_writer == kemail}">
			<input type="button" id="update" value="수정" />
		</c:if>
		...
		<!-- 삭제 권한 (작성자, 관리자)-->
		<c:if test="${dto.board_writer == sessionScope.mem_id || dto.board_writer == nname || dto.board_writer == kemail || sessionScope.mem_id == 'nanaland'}">
			<c:if test="${dto.board_re_step == 0}">  
      ...
      <div>
      	<c:choose>
         	<c:when test="${sessionScope.mem_id != null}">
            	...
            	<input type="hidden" placeholder="USER ID" id="newReplyWriter" value = "${kemail}">
         	</c:when>
         ...
         <button type="button" id="replyAddBtn">등 록</button>
      </div>      
   </div>
   ...
   <c:forEach items="${boardDTO.replyList}" var="replyDTO">
      <div class = "time_sub" id = "${replyDTO.reply_num}">
         ...
         <c:when test="${replyDTO.reply_writer == sessionScope.mem_id}">
          <div>
            <button id = "${replyDTO.reply_num}" class = "deletebutton">삭제</button>
            <button id = "${replyDTO.reply_num}" class = "updatebutton">수정</button>
          </div>
         </c:when>
         
         <c:when test="${replyDTO.reply_writer == kemail}">
          <div>
            <button id = "${replyDTO.reply_num}" class = "deletebutton">삭제</button>
            <button id = "${replyDTO.reply_num}" class = "updatebutton">수정</button>
          </div>
         ...
         <c:when test="${replyDTO.reply_writer == nname}">
          <div>
            <button id = "${replyDTO.reply_num}" class = "deletebutton">삭제</button>
            <button id = "${replyDTO.reply_num}" class = "updatebutton">수정</button>
         </div>
   ...
   <!-- Modal -->
   <div id="modifyModal">         
      <textarea placeholder="수정할 내용을 입력하세요." id="updateReplyText" style="border:0px; resize: none;"></textarea>
